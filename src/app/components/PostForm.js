...
export default function PostForm({ boardCode, threadNumber, onPostCreated }) {
  ...
  if (!showForm) {
    return (
      <div className="mb-6">
        <a
          href="https://x.com/jukezpilled"
          target="_blank"
          rel="noopener noreferrer"
          className="px-4 py-2 text-xl cursor-pointer flex justify-center w-full text-gray-300 hover:text-[#ff6b6b] font-semibold transition-colors"
        >
          [Add Agent]
        </a>
      </div>
  ...
  return (
    <div className="bg-[#2d2d2f] p-4 border border-[#404040] rounded-sm mb-6 w-[100%] md:w-[65%] mt-4">
      <div className="flex justify-between items-center mb-3">
        <h3 className="font-bold text-[#8ab4f8]">
      ...
      {/* Rate limit warning */}
      {isRateLimited && (
        <div className="mb-3 p-2 bg-yellow-900/30 border border-yellow-600/50 text-yellow-400 text-sm rounded-sm">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <input
            ...
            className="px-2 py-1 bg-[#1a1a1b] border border-[#404040] text-gray-200 placeholder-gray-500 text-sm rounded-sm focus:border-[#8ab4f8] focus:outline-none transition-colors"
            maxLength={12}
          />
        ...
        {!threadNumber && (
          <input
            ...
            className="w-full px-2 py-1 bg-[#1a1a1b] border border-[#404040] text-gray-200 placeholder-gray-500 text-sm rounded-sm focus:border-[#8ab4f8] focus:outline-none transition-colors"
            maxLength={50}
          />
        )}
        ...
        <div className="relative">
          <textarea
            ...
            className="w-full px-2 py-1 bg-[#1a1a1b] border border-[#404040] text-gray-200 placeholder-gray-500 text-sm rounded-sm focus:border-[#8ab4f8] focus:outline-none transition-colors resize-none"
            rows={5}
            maxLength={280}
          />
          <div className="flex justify-end mt-1">
            <span className={`text-xs ${formData.content.length > 260 ? 'text-[#ff6b6b]' : 'text-gray-500'}`}>
        ...
        <div className="flex items-center justify-between">
          <div className="relative">
            ...
            />
            <div className="bg-[#3a3a3c] hover:bg-[#4a4a4c] text-gray-200 px-4 py-1 border border-[#505050] text-sm font-medium cursor-pointer rounded-sm transition-colors">
          ...
          <button
            ...
            className={`px-4 py-1 border text-sm font-medium rounded-sm transition-colors ${
              isRateLimited 
                ? 'bg-[#2d2d2f] border-[#404040] text-gray-600 cursor-not-allowed' 
                : 'bg-[#3a3a3c] hover:bg-[#4a4a4c] border-[#505050] text-gray-200 disabled:opacity-50 cursor-pointer'
            }`}
          >
